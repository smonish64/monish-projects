...
resource "aws_volume_attachment" "app_attach1" {
  device_name = "/dev/sdk"
  instance_id = aws_instance.appins1.id
  volume_id   = aws_ebs_volume.appvol1.id
}
...
resource "aws_ebs_volume" "dbvol1" {
  availability_zone = "us-east-1a"
  size              = 30
  tags = {
    Name = "sec_disk_db1"
...
resource "aws_volume_attachment" "db_attach1" {
  device_name = "/dev/sde"
